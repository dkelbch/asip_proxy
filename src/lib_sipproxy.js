...
function sendCancel(rq, via, route, _reason='SIP ; cause:486;test="request terminated') {
  sip.send({
    method: 'CANCEL',
    uri: rq.uri,
    headers: {
...
function _sendCancel(ctx,rq,via,route){
  if('orig_reason' in ctx){
    sendCancel(rq, via, route, ctx.orig_reason);
  }
  else{
  ...
  sip.start(options, function(rq, remote) {
    if(rq.method === 'CANCEL') {
      ...
      if(ctx) {
        ...
        ctx.cancelled = true;
        if('reason' in rq.headers){
          ctx.orig_reason = rq.headers.reason;
        }
        if(ctx.cancellers) {
